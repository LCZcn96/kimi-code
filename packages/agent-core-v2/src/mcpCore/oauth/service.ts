 ...
 *
 * One instance per process (shared with every workspace handler and session
 * overlay). The service owns one {@link McpOAuthClientProvider} per
 * server/resource and mediates both the synthetic
 * `mcp__<server>__authenticate` tool flow and the management-plane
 * login/reset operations:
 *
